    ...
    async def start_device_polling(self) -> None:
        while True:
            ...
            for device in newly_discovered_devices:
                wrapped_bed = MqttSutaBed(BleSutaBed(device))
                if device.address in self.mqtt_bridge.known_devices:
                    ...
                    await wrapped_bed.bed._ensure_connection()
                    await self.mqtt_bridge.add_tracked_device(key = device.address, device = wrapped_bed)
                elif not device.address in self.mqtt_bridge.unpaired_devices:
                    # This is a device we have not seen before. Add it as unpaired.
                    await self.mqtt_bridge.add_unpaired_device(key = device.address, device = wrapped_bed)
